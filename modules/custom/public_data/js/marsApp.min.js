		...
		controller: ["$scope", function($scope){
			...
			function refreshData(updateResponsive){
				$timeout(function(){
                    ...
                    // Search
                    if($scope.tableOptions.searching && $scope.gridState.search && $scope.gridState.search.text && $scope.gridState.search.text.trim() != '') {
                        if($scope.gridState.search.column){
                            originalRecords = _.filter(originalRecords, function(record){
                                var recordValue = record[$scope.gridState.search.column.attrId] || '';

                                return recordValue && (recordValue+"").toLowerCase().indexOf($scope.gridState.search.text.toLowerCase()) >= 0;
                            })
                        } else{
                            originalRecords = _.filter(originalRecords, function(record){
                                for(var prop in record) {
                                    var recordValue = record[prop] || '';
                                    if(recordValue && (recordValue + "").toLowerCase().indexOf($scope.gridState.search.text.toLowerCase()) >= 0) {
                                        return true;
                                    }
                                }
                                return false;
			...
			function searchReset(){
				$scope.gridState.search.column = null;
				$scope.gridState.search.text = null;
				refreshData(true);
			}

			function searchCallBack($attrId, $searchValue){
				if(!$scope.gridState.search) $scope.gridState.search = {};
				if($attrId) $scope.gridState.search.column = _.find($scope.tableColumns, {'attrId': $attrId});
				$scope.gridState.search.text = $searchValue;
				refreshData(true);
			}
            ...
            if($scope.tableOptions.searching) {
                $scope.$on('datagrid.search', function (event, param) {
                    // if($scope.searchControlId === param.searchControlId) { 
                    // }
                    searchCallBack(param.attrId, param.searchValue);
                }); 
            }
...
app
.directive('dcSearchBox', ["CommonFactory", "$timeout", function (CommonFactory, $timeout){
	return {
		replace: true,
...
app.directive('sectionFilter',["$parse", "DataFactory", "$q", "$timeout", "$rootScope", function($parse, DataFactory, $q, $timeout, $rootScope){
	return {
        restrict : 'E',
        replace : true,
...
app.factory('DataFactory', ["CommonFactory", "BaseFactory", "$http", "$q", function DataFactory(CommonFactory, BaseFactory, $http, $q) {
    ...
    var baseUrl = "//10.80.9.9/services/v1/";
